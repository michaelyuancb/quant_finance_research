...
def orth_panel_global_symmetry(df_org, df_column, column, top_k=None, eps_bar=1e-6, orth_matrix=None, **kwargs):
    """
        S = O * D^{-1/2} * O
    """
    df = copy.deepcopy(df_org)
    factor_data = df.iloc[:, column]
...
def orth_panel_global_standard(df_org, df_column, column, top_k=None, eps_bar=1e-6, orth_matrix=None, **kwargs):
    """
        S = O * D^{-1/2}
    """
    df = copy.deepcopy(df_org)
...
def orth_panel_global_custom(df_org, df_column, column, custom_transform=None, top_k=None, eps_bar=1e-6,
...
def orth_panel_global_residual(df_org, df_column, column, orth_coef_list=None, **kwargs):
    """
...
def orth_panel_local_residual(df_org, df_column, column, orth_column_base, orth_coef_list=None, **kwargs):
    """
        perform the orthogonalization by linear regression (alternative with residual).
